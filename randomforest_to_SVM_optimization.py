...
training = pd.read_csv('Groceries_Processed_Training_Data.csv', nrows=1000)
del training['Unnamed: 0']

# read processed Test csv file with only 1 row per product
test = pd.read_csv('Groceries_Processed_Test_Data.csv', nrows=1000)
del test['Unnamed: 0']
...
summary_model = GridSearchCV(RandomForestRegressor(random_state=3), param_grid={
...
print(summary_model.best_params_)
print(summary_model.best_score_)

# create RF model based on bag of words for combined reviewTexts of each product
body_model = GridSearchCV(RandomForestRegressor(random_state=3, n_estimators=100), param_grid={
                                                        'n_estimators': (100, 200, 500, 1000),
                                                        'max_features': ['auto', 'sqrt', 'log2'],
                                                        'bootstrap': [True, False],
                                                        'max_depth': [None, 5, 10, 25, 50, 100],
...
print(body_model.best_params_)
print(body_model.best_score_)
